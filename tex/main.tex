...
In informatica un tipo è un vincolo che definisce un insieme ben definito di valori validi per una risorsa di un programma.
...
Un programma che supera il controllo dei tipi (definizione \ref{def:type_checking}) è ben tipato e un linguaggio in cui nessun programma ben tipato genera errori trapped né untrapped si dice \textbf{sound} e in un linguaggio \textbf{safe} nessun programma genera errori untrapped, un linguaggio sound è necessariamente type safe ~\cite{cencia:dispense}.
...
\begin{myDefinition}{Type checking}{type_checking}
    È la verifica dei vincoli imposti dai tipi, può avvenire sia a tempo di compilazione che a tempo di esecuzione e si parla rispettivamente di type checking statico/forte e dinamico/debole.
