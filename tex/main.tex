...
Rust è un linguaggio di programmazione che si focalizza sulla velocità e sulla sicurezza dei suoi programmi.
La semantica stessa del linguaggio definisce delle regole che impediscono l'insorgere di programmi non sicuri, tendenzialmente un bug nasce dalla distrazione del programmatore e Rust adottando dei semplici accorgimenti riduce notevolmente la possibilità che accadano questi errori, ad esempio anche il solo fatto che le variabili siano modificabili solo quando esplicitamente richiesto è in accordo con il \textbf{principle of least privilege}.

\section{Principi di sicurezza}
Nello sviluppo di sistemi sicuri ci si attiene a dei principi, non è sempre possibile rispettarli tutti e alle volte vanno in contrasto tra loro né è sempre banale individuare come un sistema si ponga rispetto ad uno dei principi ~\cite{gollmann:computersecurity}.

\begin{itemize}
	\item \textbf{Least privilege.} Ad un soggetto è garantito l'insieme minimale di privilegi necessari per completare la sua task.
	\item \textbf{Fail-safe default.} Ad un soggetto è consentito l'accesso ad una risorsa solo se dichiarato esplicitamente.
	...
	\item \textbf{Open design.} La sicurezza di un meccanismo non dipende dalla sua segretezza, il codice non deve essere necessariamente pubblico ma la \textit{security by obscurity} non è abbastanza. 
		Ad esempio il funzionamento di sistemi di cifratura a chiave asimmetrica è di pubblico dominio, il sistema è sviluppato per essere sicuro (\textit{security by design}). 
		Il principio di open design non si applica ad informazioni che devono essere segrete quali chiavi crittografiche o d'accesso.
	\item \textbf{Separation of privilege.} Un sistema non deve garantire i permessi basandosi su una singola condizione.
...
Rust regola l'accesso in conformità a questi principi tramite \textbf{ownership} e \textbf{borrowing}, sono regole molto semplici ed eleganti che però richiedono maggiore impegno da parte del programmatore.
I principali problemi di Rust sono la sua difficoltà e la curva d'apprendimento, la stragrande maggioranza di programmatori ha più familiarità con linguaggi imperativi e Rust per quanto condivida una sintassi simile al C prende a piene mani da linguaggi funzioanli come ML e Haskell. 
Rust eredita il \textcode{let} dai linguaggi funzionali e quasi ogni parte di una funzione è un'espressione ~\cite[1309-1573]{rust:syntax}, nel listato \ref{lst:funrs_if} si può vedere come una condizionale sia anche un'espressione, non c'è necessità dell'operatore ternario \textcode{?}.
Anche il \textcode{match} è preso direttamente dai linguaggi funzionali, nel listato \ref{lst:funrs_match} si nota anche come non sia necessario specificare un valore di ritorno, omettendo il tipico \textcode{;} si ha un return automatico come in LISP.
...
L'ownership rappresenta il possesso di un \textit{right value} da parte di un \textit{left value}, in Rust gli oggetti di tipo composto sono lineari e solo a questi si applica ownership, al momento dell'inizializzazione di un oggetto la sua variabile, se presente, è l'unico owner e un assegnamento ad una seconda variabile fa passare il possesso dalla prima alla seconda: in qualsiasi momento quindi può esserci solo un possessore per ogni oggetto. 
...
La garbage collection è una delle tecniche di gestione automatica della memoria più usate nei linguaggi di programmazione: tramite un modulo noto come \textbf{garbage collector} si tiene traccia di ogni allocazione e le zone di memoria non più necessarie vengono periodicamente liberate, rendendole nuovamente disponibili. 
...
Ad oggi differenti linguaggi utilizzano tecniche differenti per i loro meccanismi di sicurezza, è consuetudine che il linguaggio presenti un sistema di tipi.
Il controllo sui tipo vieta automaticamente una gamma di azioni che coinvolgono oggetti di tipo differente, nonostante ciò non è comunque abbastanza dato che C è tipato e non è sicuro.
Infatti è possibile oltrepassare alcuni limiti imposti dai tipi con i puntatori, la maggior parte dei linguaggi odierni considerati sicuri eliminano questo problema nascondendo i puntatori al programmatore e nel caso in cui fosse presente un'aritmetica dei puntatori di norma non sono raw pointer come in C ma smart pointer, questi ultimi presentano in più rispetto ai primi delle funzioni in più quali bound check e possono tenere traccia dell'uso della memoria.
Ad un controllo sui tipi è quindi necessario un controllo sugli accessi, ad esempio in Java esistono classi con più permessi di altre ~\cite{java:privilege}, in Rust è possibile definire una propria politica di autorizzazioni d'accesso ~\cite{rust:auth} invece in Python non è proprio possibile ~\cite{python:privilege}.

\section{Obiettivi}
L'obiettivo di questo documento è verificare il comportamento di Rust di fronte a tipiche violazioni della sicurezza nei linguaggi di programmazione e tal fine è stato preso come metro di paragone il linguaggio C, noto per essere non sicuro, ed è stato scritto del codice non sicuro.
Poi è stato scritto del codice che potesse avere effetti simili in Rust osservandone il risultato.

Nel capitolo \ref{chap:security_policy} è data la definizione che Rust dà di sicurezza, per analizzare i vari tipi di violazioni di codice è stata fatta una distinzione tra memory safety \ref{chap:memory_safety} e type safety \ref{chap:type_safety}.
La proprietà sicurezza in Rust è garantita da ownership e borrowing, nel capitolo \ref{chap:Rust} viene approfondito il sistema che ne è alla base.
...
Rust offre le seguenti proprietà:
\begin{enumerate}
	\item Nessun programma genera undefined behaviour.
	\item Non è possibile dereferenziare un raw pointer.
	\item Non è possibile leggere né scrivere una variabile statica che sia anche \textcode{extern} o \textcode{mut}.
	\item Non è possibile accedere ad un campo di un'\textcode{union} se non per l'assegnamento.
	\item Presentare codice \textcode{unsafe}
\end{enumerate}

\chapter{Rust} \label{chap:Rust}
I meccanismi di sicurezza in Rust sono tanto solidi perché ben definiti matematicamente.
Un'interpretazione della logica lineare di Girard permette di esprimere formalmente operazioni in termini di consumo di risorse, questa interpretazione è implementata nell'ownership per la quale l'"uso" equivale alla liberazione al termine del proprio scope e per evitare double free non possono esistere due variabili che possiedono la stessa risorsa.
Un sistema del genere è detto substrutturale e nel caso specifico di Rust si fa uso dei \textbf{lifetime}, è possibile definirne formalmente il comportamente.
